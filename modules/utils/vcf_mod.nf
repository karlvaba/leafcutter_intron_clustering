...
process modify_vcf_pr {
    ...
    output:
    path "${vcf.simpleName}.modified.vcf.gz", emit: vcf_modified

    shell:
    '''
    bcftools view -Oz -s `bcftools query -l !{vcf} | head -n 1` !{vcf} | gunzip | sed -e 's/[0-9]|[0-1]/0|1/g' | gzip -c > !{vcf.simpleName}.modified.vcf.gz
    '''
}
